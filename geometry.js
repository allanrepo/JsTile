...
function vector(x, y)
{
	this.log = function(){ console.log(x + ", " + y); }
	...
	if (x instanceof vector){ y = x.y(); x = x.x(); }
	
	// if first argument isn't vector, check if x, y arguments are numbers
	if (typeof x != 'number') x = 0;	
	if (typeof y != 'number') y = 0;	

    this.copy = function(V){ x = V.x(); y = V.y(); return this; }    	
	
	// vector operations
    this.dot = function(V){ return x*V.x() + y*V.y(); }
    this.magnitude = function(){ return Math.sqrt(x*x + y*y); }
    this.normalize = function(){ var m = this.magnitude(); x /= m; y /= m; }
	
	// transformations
	this.translate = function(tx, ty)
	{
		if (typeof tx == 'undefined' && typeof ty == 'undefined') return; 
		if (tx instanceof vector){ y += tx.y(); x += tx.x(); return; }
		if (typeof x == 'number') x += tx;	
		if (typeof y == 'number') y += ty;		
	}
    ...
    this.rotate = function(r)
    {
		...
		var rx = x;
		var ry = y;
		x = rx*Math.cos(r) - ry*Math.sin(r);
		y = ry*Math.cos(r) + rx*Math.sin(r);
    }	
	
	this.scale = function(sx, sy){ x *= sx;	y *= sy; }

	
	
	// math operations
    this.sub = function(V){ if (!(V instanceof vector)) return;	return new vector(x - V.x(), y - V.y()); }	
    this.add = function(V){ if (!(V instanceof vector)) return; return new vector(x + V.x(), y + V.y()); }           	
    this.mul = function(a){ if (typeof a != 'number') return; return new vector(x*a, y*a); }	
    this.div = function(a){ if (typeof a != 'number') return; return new vector(x/a, y/a); }               
	
	
    this.vector = function(V){ return this.sub(V); }    
